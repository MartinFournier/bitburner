...
const optionDefinitions = [
  ...
  { name: 'simple', alias: 's', type: Boolean, default: true},
];
...
class MergeChangelog {
  ...
  async getChangelog(from, to, simple) {
    const changes = await this.getPullsMergedBetween(from, to);
    const pullLines = changes.pulls.map((line) => this.getPullMarkdown(line, simple));
    const commitLines = changes.danglingCommits.map((line) => this.getCommitMarkdown(line, simple));
    const title = `[draft] v1.x.x - ${changes.to.date.toISOString().split('T')[0]}`;
    let log = `
${title}
${'-'.repeat(title.length)}
...
Modifications included between ${changes.from.date.toISOString().split('T')[0]} and ${changes.to.date.toISOString().split('T')[0]} (${changes.from.commit.sha.slice(0, 7)} to ${changes.to.commit.sha.slice(0, 7)}).
...
`;
  ...
  getCommitMarkdown(commit, simple) {
    if (simple) {
      return `* ` +
...
api.getChangelog(cliArgs.from, cliArgs.to, cliArgs.simple).then((data) => {
